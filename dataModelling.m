classdef dataModelling < handle
    ...
    methods(Access = public)
        ...
        function [err, simdat, pred, DecValue] = leakyAccumulator(obj, pm, parNames, noise, getCPP)
            ...
            bound = []; drift = []; leak = []; tnd = []; boost = [];  % TODO add trialnoise = [];
            ...
            for n = 1:size(noise,3) % for robustness, it can help to simulate more trials than there are in the real data...
                for indBlock = 1:length(TOW)
                    ...
                    for indTarget = 1:length(targT)
                        ...
                        if getCPP == 1
                            if (targT(indTarget)/dt) + ceil((obj.stim.targetEpoch(end))/dt) < length(DV)
                                DecValue.Target(:,acc) = DV(round(targT(indTarget)/dt) + [floor((obj.stim.targetEpoch(1))/dt):ceil((obj.stim.targetEpoch(end))/dt)]);
                                try
                                    if ~isempty(nextrespind)
                                        DecValue.Response(:,acc) = DV(round(RespT(nextrespind)/dt) + [floor((obj.stim.responseEpoch(1))/dt):ceil((obj.stim.responseEpoch(end))/dt)]);
                                    else
                    ...
                    for indITI = 1:length(ITIstartT)-1
                        ...
                        nextrespind = find(RespT > ITIstartT(indITI)+maxRTms-(obj.stim.duration) & RespT < endtime); % find indices of responses the ITI window, ruling out any at the very start that are within the hit window from the previous target. Target duration is 1 sec, so fs in sample points
