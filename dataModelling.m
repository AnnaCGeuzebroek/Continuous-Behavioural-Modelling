classdef dataModelling < handle
    ...
    methods(Access = public)
        ...
        function [bestpmFits, EE] = applyModelling(obj, modelName, initialGuess, modelPar, numRepeats, jitterPar, freeParameters)
            ...
            if isempty(initialGuess) || exist('freeParameters', 'var')
                ...
                if exist('freeParameters', 'var')
                    ...
                    for indRow = 1:size(freeParameters,1)
                        if isempty(jitterPar) || (indRow == 1 && size(freeParameters,1) > 1)
                            newParameters(:,ismember(index, freeParameters(indRow,:)))  = generateRandParameters(obj, modelPar(freeParameters(1,:),:), numRepeats);
                        else
                            newParameters(:,ismember(index, freeParameters(indRow,:)))  = jitterPar(randi(size(jitterPar,1),numRepeats,1),:) +  (randn(numRepeats, size(jitterPar,2)).*mean(jitterPar).*0.15);
                        end
                    end
            ...
            if ~exist(fullfile(saveFolder, [saveFile '.mat']), 'file')
                ...
                for indPM = 1:numInitialGuess
                    
                    fprintf('start pt %i:', indPM)
                    [pm, Err] = fminsearchbnd(@(pm) leakyAccumulator(obj, pm, modelNames, noise), initialGuess(indPM,:), lowerBound, upperBound,  obj.options);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %-----------------  Leaky accumulation  ---------------------------
        function [err, simdat, pred, DecValue] = leakyAccumulator(obj, pm, parNames, noise, getCPP)
            ...
            for n = 1:size(noise,3) % for robustness, it can help to simulate more trials than there are in the real data...
                for indBlock = 1:length(TOW)
                    ...
                    if isempty(leak); currLeak = 0; elseif length(leak) > 1; currLeak = leak(bt(indBlock,1)); else; currLeak = leak; end
                    
                    if isempty(boost) || length(boost) > 1
                        currBoost = 1;
                    elseif ~isempty(boost) && length(boost) == 1
                    ...
                    timeLine(timeLine ~= 0) = currDrift(abs(timeLine(timeLine ~= 0))).*currBoost;
                    if obj.DetectOrDisc
                        direction = TOW{indBlock}(:,1)';
                        timeLine(direction < 1) = timeLine(direction < 1)*-1;
                    end
                    
                    sensEv = timeLine + noise(indBlock,1:length(TOW{indBlock}), n);
                    ...
                    for indTime = round(tnd/dt)+1:length(sensEv)
                        ...
                        if indTime <= lastresp+continAccDur || indTime > lastresp + postRespPause % don't accumulate unless it has been a sufficient time since last response
                            DV(indTime) = (1-currLeak)*DV(indTime-1) + sensEv(indTime-round(tnd/dt));% TODO add - currCrit % There is the main model equation! (see e.g. Ossmy et al 2013)
                        else
                    ...
                    for indTarget = 1:length(targT)
                        ...
                        if getCPP == 1
                            if (targT(indTarget)/dt) + ceil((obj.stim.targetEpoch(end)./1000)/dt) < length(DV)
                                DecValue.Target(:,acc) = DV(round(targT(indTarget)/dt) + [floor((obj.stim.targetEpoch(1)./1000)/dt):ceil((obj.stim.targetEpoch(end)./1000)/dt)]);
                                try
                                    if ~isempty(nextrespind)
                                        DecValue.Response(:,acc) = DV(round(RespT(nextrespind)/dt) + [floor((obj.stim.responseEpoch(1)./1000)/dt):ceil((obj.stim.responseEpoch(end)./1000)/dt)]);
                                    else
                    ...
                    ITIstartT = [0 targT+obj.stim.duration/1000];
        ...
        function [pred, currPar] = plotModellingResults(obj, bestpm, modelName, parNames, err, simdat, DV)
            ...
            % TODO add stuff to log, e.g. add fit parameters and goodness
            % of fits.
            ...
            MT = 4;
            targetEpoch = ((obj.stim.targetEpoch(1)/1000):dt:(obj.stim.targetEpoch(end)/1000)+dt)*1000;
            responseEpoch = ((obj.stim.responseEpoch(1)/1000):dt:(obj.stim.responseEpoch(end)/1000)+dt+dt)*1000;
            
            baselineCorrect = targetEpoch >= obj.stim.baselineCorrect(1) & targetEpoch <= obj.stim.baselineCorrect(2) ;% obj.stim.baselineCorrect(1) &  targetEpoch <= obj.stim.baselineCorrect(2);
            
            noise = obj.modelBehaviour.noiseSTD*randn(length(obj.modelBehaviour.datsum.TOW), max(obj.modelBehaviour.datsum.maxln), obj.modelBehaviour.simulateMoreX); % note the 0.1* because assuming s = 0.1. Not done for NI models below
            
            if ~(exist('err', 'var') && exist('simdat', 'var'))
                [err, simdat, ~, DV] = leakyAccumulator(obj, bestpm, parNames, noise,1);
            end
            ...
            if obj.DetectOrDisc
                cond       = simdat(:,1:end-3);  uniComb = unique(cond(all(cond ~= 0,2),:), 'rows');
                outcome    = simdat(:,end-2);
                Response   = simdat(:,end);
                RT         = simdat(:,end-1);
            else
                cond       = simdat(:,1:end-2);  uniComb = unique(cond(all(cond ~= 0,2),:), 'rows');
                outcome    = simdat(:,end-1);
                RT         = simdat(:,end);
            end
            
            meanTarget    = nan(size(DV.Target, 1), length(uniComb));
            meanResponse  = nan(size(DV.Response, 1), length(uniComb));
            keyboard
            for indCond = 1:length(uniComb)
                clear tmp*
                currNumTrials = sum(all(cond(outcome ~=  3,:) == uniComb(indCond,:), 2)); % taking FA in withit

                currCond = find(all(cond == uniComb(indCond,:), 2)); % & ~isnan(obj.behaviour{indPP}.RT));
                currRT   = RT(currCond);
                currOutcome = outcome(currCond);
                
                currNumTrials = sum(currOutcome ~= 3);
                
                currTarget = DV.Target(:,currCond(currOutcome == 1));
                tmpRT = currRT(currOutcome == 1);
                
                for indEpoch = 1:length(tmpRT)
                    tmpTarget(:,indEpoch) = currTarget(:, indEpoch);
                    tmpTarget(:,indEpoch) = tmpTarget(:,indEpoch);%as  - nanmean(currTarget(baselineCorrect, indEpoch),1);
                end
                
                meanTarget(:,indCond)   = nanmean(tmpTarget,2);
                meanResponse(:,indCond) = nanmean(DV.Response(:,currCond(currOutcome == 1)),2);
                
                correctRT = currRT(currOutcome == 1);
                ...
                if ~isnan(correctRT)
                    ...
                    pred.q(indCond,:) = [0 quantile(correctRT,qps)-0.5/60 obj.stim.RTdeadLine(end)];  % subtracted half of a refresh cycle just because response can only happen on each refresh, and setting the boundaries between quantile bins is then safer so > and >= don't give different results
                    
                    for indBin =1:length(pred.q)-1
                        % number of trials in each of the 6 quantile bins
                        pred.qn(indCond,indBin) = sum(correctRT > pred.q(indCond,indBin) & correctRT <= pred.q(indCond,indBin+1));
                        % median RT within each quantile bin
                        pred.qm(indCond,indBin) = median(correctRT(correctRT > pred.q(indCond,indBin) & correctRT <= pred.q(indCond,indBin+1)));
                        ...
                        pred.pij(indCond,indBin) = pred.qn(indCond,indBin)/(currNumTrials);
                    end
                    
                    if isnan(datsum.pij(indCond,length(pred.q)))
                        pred.qn(indCond,length(pred.q)) = nan;
                        pred.pij(indCond,length(pred.q)) = nan;
                        continue;
                    end % just because there is one cell of the real and simulated data that is empty - there is no such thing as false alarm number for a "second" coherence level
                    
                    % e.g. TODO do you want to include False alarms.
                    % then add a final bin that counts the false alarms:
                    pred.qn(indCond,length(pred.q))= sum(currOutcome == 2);
                    pred.pij(indCond,length(pred.q))= sum(currOutcome == 2)/(currNumTrials);

                    pred.qn(indCond,length(pred.q)+1)= sum(currOutcome == 3);
                    ...
                    pred.pij(indCond,length(pred.q)+1)   = sum(currOutcome == 3)/(datsum.nShortestITI(indCond)*obj.modelBehaviour.simulateMoreX);
            ...
            if obj.modelBehaviour.ChiOrG == 1
                ...
                for indCond = 1:length(uniComb)
                    ...
                    legendThis = plot(datsum.qm(indCond,:),cumsum(pred.qn(indCond,1:end-1)),'-',...
                        'Color',obj.figLayOut.colours(indCond,:),'LineWidth', obj.figLayOut.lineWidth);
                end
            ...
            elseif  obj.modelBehaviour.ChiOrG == 2
                ...
                for indCond = 1:length(uniComb)
                    ...
                    legendThis = plot(pred.qm(indCond,:),cumsum(pred.pij(indCond,1:end-2)), 'LineStyle', obj.figLayOut.lineType{1},...
                        'Color',obj.figLayOut.colours(indCond,:),'MarkerSize', 3, 'LineWidth', 1);
            ...
            if obj.figLayOut.saveDim == [4 6]
                ...
                set(gca,'XTickLabel',[0:0.5:obj.stim.RTdeadLine(end)]);
            else
                xlim([0.1 1.25])
                set(gca,'XTick', [0.2:0.2:obj.stim.RTdeadLine(end)])
                set(gca,'XTickLabel',[0.2:0.2:obj.stim.RTdeadLine(end)]);
            end
            ...
            %{
            title(sprintf('%s (AIC: %0.1f, BIC: %0.1f)', modelName, AIC, BIC))
            
            printText = strcat(parNames, ' = ', num2str(pm',3));
            for indText = 1:size(printText,1)
            	text(obj.stim.RTdeadLine(end)-0.3, 0.7 - (0.6/size(printText,1)*indText), printText(indText,:))
            end
            %}
            ...
            if obj.modelBehaviour.ChiOrG == 1
                ...
                %{
                for indCond = 1:length(uniComb)
                    bb = bar(indCond, pred.qn(indCond,end));
                    set(bb,'FaceColor', obj.figLayOut.colours(indCond,:), 'FaceAlpha', 0.3)
                    
                    plot(indCond, cumsum(datsum.qn(indCond,end)),'*',...
                        'Color',obj.figLayOut.colours(indCond,:),'LineWidth', obj.figLayOut.lineWidth)
            ...
            elseif  obj.modelBehaviour.ChiOrG == 2
                ...
                for indCond = 1:length(uniComb)
                    ...
                    bb = bar(indCond, pred.pij(indCond,end));
                    set(bb,'FaceColor', obj.figLayOut.colours(indCond,:), 'FaceAlpha', 0.3)
                    errorbar(indCond, pred.pij(indCond,end), CI95,'k','LineWidth',obj.figLayOut.lineWidth)
                    
                    plot(indCond, cumsum(datsum.pij(indCond,end)),'o',...
            ...
            plotSave(gcf, ['QuantileProbFA' modelName '.png'], figureFolder,  obj.figLayOut.saveDim);
            ...
            %{
            figInfo = panel();
            figInfo.pack(1, {65 35});
            figInfo(1,1).marginright = 4;
            %             figInfo(1,2).marginleft  = 4;
            
            figInfo.marginbottom = 10;
            figInfo.marginright  = 10;
            figInfo.fontsize = obj.figLayOut.letterSize;
            figInfo.fontname = obj.figLayOut.letterType;
            %}
            acc = 1;
            for indCond = 1:size(meanTarget,2)
                ...
                baseline = nanmean(plotTarget(baselineCorrect,:),1);
                
                plotTarget   = plotTarget- baseline;
                plotResponse = plotResponse- baseline;
                
                YStuff(acc,1) = min([plotTarget]); %; plotResponse
                YStuff(acc,2) = max([plotTarget]); %; plotResponse
                
                %% plot Target-locked epochs
                figure(figHandle)
                % figInfo(1, 1).select(); hold on;
                ...
                grid on
                xstuff = [fliplr(0:-500:obj.stim.targetEpoch(1)) 500:500:obj.stim.targetEpoch(end)];
                ...
                % figInfo(1, 2).select(); hold on;
                figure(figResponse)
               
                YStuffR(acc,1) = min(plotResponse); %; plotResponse
                YStuffR(acc,2) = max(plotResponse); %; plotResponse
                
                % plot evidence onset
                
                h = plot(responseEpoch(responseEpoch < 3*dt*-1000), plotResponse(responseEpoch < 3*dt*-1000));
                h.Color = obj.figLayOut.colours(acc,:);
                h.LineWidth = obj.figLayOut.lineWidth;
                h.LineStyle = obj.figLayOut.lineType{acc};
                forComparision(acc) = plotResponse(find(responseEpoch >= 4*dt*-1000,1));
                acc = acc + 1;
            ...
            ylim([-0.05 1]);
            line([0 0], [-0.05 1], 'Color', 'k', 'LineWidth', 2)
         
            xlim([0 1000])
            xticks([0:500:1000])
            xticklabels([0:500:1000]./1000)
            ...
            limits = [min(YStuffR(:,1)) max(YStuffR(:,2)).*1.02];
            % plot Response
            figure(figResponse)
            currAx = gca;
            ...
            if ~isempty(obj.figLayOut.responseLim)
                ...
                xstuffResponse = [-500 0 200];
            end
            
            xlim([xstuffResponse(1) 0])
            xticks(xstuffResponse)
            xticklabels(xstuffResponse/1000)
            ...
            %             line([xstuffResponse(1) 0], [currPar.Value(1) currPar.Value(1)], 'Color',obj.figLayOut.colours(1,:), 'LineStyle', '--')
            %             line([xstuffResponse(1) 0], [currPar.Value(2) currPar.Value(2)], 'Color',obj.figLayOut.colours(2,:), 'LineStyle', '--')
            %             line([xstuffResponse(1) 0], [currPar.Value(3) currPar.Value(3)], 'Color',obj.figLayOut.colours(4,:), 'LineStyle', '--')
            ...
            plot([1 2], forComparision(1:2), 'LineWidth', obj.figLayOut.lineWidth, 'LineStyle','-.',...
                'Color', obj.figLayOut.colours(2,:), 'Marker', '+')
            plot([1 2], forComparision(3:4), 'LineWidth', obj.figLayOut.lineWidth, 'LineStyle', '-.',...
...
%             ylabel({' ' ' '})
            ...
            plotSave(gca, ['preResponse' modelName 'Response.png'], figureFolder,  [4 2.5]);
...
%             load(fullfile(obj.outputFolder, 'meanTargetCPP.mat'), 'meanTarget')
%             meanTarget = resample(squeeze(nanmean(meanTarget,4)),length(targetEpoch), size(meanTarget,2) );
            ...
            for indCond = 1:size(meanTarget, 2)
                diffmeanTarget(:,indCond) = diffmeanTarget(:,indCond) - nanmean(diffmeanTarget(targetEpoch > -50 & targetEpoch < 50,indCond),1);% nanmean(diffmeanTarget(targetEpoch > -100 & targetEpoch < 0,indCond),1);
            end
            ...
            for indCond = 1:size(meanTarget,2)
                ...
                % plotTarget = plotTarget - nanmean(plotTarget(1:2,:),1);
                % plotResponse = plotResponse - nanmean(plotResponse(1:2,:),1);
                
                YStuff(acc,1) = min([plotTarget]);% ; plotResponse
                YStuff(acc,2) = max([plotTarget]);
                ...
                % plot evidence onset
                h = plot(responseEpoch, plotResponse);
                h.Color = obj.figLayOut.colours(acc,:);
                h.LineWidth = obj.figLayOut.lineWidth;
                h.LineStyle = obj.figLayOut.lineType{acc};
            ...
            %%
            simTarget = meanTarget;     
            simTarget = diff(simTarget);
            %%
            load(fullfile(obj.outputFolder, 'EEG data', 'groupAverage', 'CPP', ['CPPBestElec_HPF0_CSD1.mat']), 'ERPTarget')
            clear meanTarget
             
            [within, ~, ~, allTrialMatrix] = getConditions(obj, [4 2]);
            save('./SupportingCode/ForBootstrap.mat', 'within', 'allTrialMatrix');
            
            Fs    = 512;%obj.eeg.SampleRate;
            fco   = 6;
            [b,a] = butter(2,fco*2/Fs);
            
            for indPP = 1:length(obj.ppNames)
                meanTarget(:,:,indPP) = filtfilt(b,a, squeeze(ERPTarget(:,:,:,indPP)));
            end
            
            [~,indSim] = max(diff(simTarget));
            indSim = indSim(1);
            clear Target
            for indPP = 1:size(meanTarget,3)
                 Target(:,:,indPP)      = diff(meanTarget(:,:,indPP));

                for indCond = 1:size( within.Condition,1)
                    currCond   = find(all(allTrialMatrix(:,:,indPP) ==  within.Condition(indCond,:), 2) == 1);
                    tmpTarget  = nanmean(Target(:,currCond,indPP),2);
                    
                    forPlot(:,indCond, indPP) = tmpTarget;
                end
            end
            forPlot = forPlot(:,[2 4],:)- forPlot(360,[2 4],:)+1;
            realData = squeeze((forPlot(360,:,:)-forPlot([360+50],:,:))./ forPlot(360,:,:)*100)';
     
            [bootstat, bootsam] = bootci(10000, {@mean, realData}, 'Alpha', 0.01, 'type', 'per');
            nanmean(bootsam)
            
            simTarget  = (simTarget -  min(simTarget(:)))/ (max(simTarget(:)) - min(simTarget(:)));
            precDecay = ((simTarget([indSim+2],[2 4]) -  simTarget([indSim+8],[2 4]))./ simTarget([indSim+2],[2 4]))*100;
       
            keyboard
% keyboard
% 
%             [bootstat, bootsam] = bootstrp(1000, @signTesting, realData(:,2), precDecay(1));
%             figure, hist(bootstat, 100) 
%             [~, pValues, ~, test] = ttest( realData(:,2), precDecay(2));
% 
%              line(repmat(test.tstat,2), [0 25])
%             keyboard
%             addGOFString = char(['Dec' repmat(' ',  1, length(currPar.Parameter(1,:))-3)]);
%             
%             currPar = [currPar; table(addGOFString, [precDecay(1)]', 'VariableNames', {'Parameter', 'Value'})];
%             addGOFString = char(['Dec' repmat(' ',  1, length(currPar.Parameter(1,:))-3)]);
%             
%             currPar = [currPar; table(addGOFString, [precDecay(2)]', 'VariableNames', {'Parameter', 'Value'})];
        end
        ...
        function datsum = plotRTquantiles(obj, plotComb)
            ...
            numBins = length(qps) + 1; furtherIndex = numBins; 
            for indPP = 1:length(obj.ppNames)
                ...
                for indCond = 1:length(uniCond)
                    ...
                    currCond   = find(all(currTrials == uniCond(indCond,:), 2));% & ~isnan(obj.behaviour{indPP}.RT));
                   ...
                   if obj.DetectOrDisc
                    ...
                    qn(indCond,furtherIndex,indPP)   = sum(currMisses);
            ...
            datsum.nShortestITI    = nanmean(nShortestITI,2);

            datsum.pij = datsum.qn./datsum.numberOfTrials;
            datsum.pij(:,furtherIndex+1) = datsum.qn(:,furtherIndex+1)./datsum.nShortestITI;
            ...
            elseif  obj.modelBehaviour.ChiOrG == 2
                ...
                for indCond = 1:size(datsum.q, 1)
                    %	plot(datsum.qm(indCond,:),cumsum(datsum.pij(indCond,1:end-1)),'*',...
                    %	'Color',obj.figLayOut.colours(indCond,:),'LineWidth', obj.figLayOut.lineWidth)
                    ...
                    h = errorbar(datsum.qm(indCond,1:numBins), cumsum(datsum.pij(indCond,1:numBins)),CI95,...
                        ...
                        'LineWidth', obj.figLayOut.lineWidth);
                    %{
                    h = shadedErrorBar(datsum.qm(indCond,:), cumsum(datsum.pij(indCond,1:end-1)),CI95);
                    h.mainLine.Color = obj.figLayOut.colours(indCond,:);  h.patch.FaceColor = obj.figLayOut.colours(indCond,:);
                    h.edge(1).Color  = obj.figLayOut.colours(indCond,:);  h.edge(2).Color   = obj.figLayOut.colours(indCond,:);
                    h.mainLine.LineWidth = 1.2; h.edge(1).LineStyle = 'none'; h.edge(2).LineStyle = 'none';
                    h.patch.FaceAlpha = obj.figLayOut.plotCI;
                    
                    h.mainLine.LineStyle = obj.figLayOut.lineType{indCond};
                    %}
                        legendThis(indCond) = h;
                end
        ...
        function [err, pred] = Chisquared(obj, simulatedData)
            ...
            datsum     = obj.modelBehaviour.datsum;
            cond       = simulatedData(:,1:end-2); uniComb = unique(cond, 'rows');
            outcome    = simulatedData(:,end-1);
            RT         = simulatedData(:,end);
        ...
        function [err, pred] = Gsquared(obj,simulatedData)
            % Now count the number of simulated trials falling in each of the quantile bins of the real data.
            ...
            if obj.DetectOrDisc
                cond       = simulatedData(:,1:end-3); 
                outcome    = simulatedData(:,end-2);
                RT         = simulatedData(:,end-1);
                Response   = simulatedData(:,end);
                Response(isnan(Response)) = 0;
            else
                cond       = simulatedData(:,1:end-2);
                outcome    = simulatedData(:,end-1);
                RT         = simulatedData(:,end);
            end
...
keyboard
                % then add a final bin that counts the false alarms:
                % if isnan(datsum.pij(indCond,indBin+2)),  pred(indCond,indBin+1) = nan; continue; end % just because there is one cell of the real and simulated data that is empty - there is no such thing as false alarm number for a "second" coherence level
                pred(indCond,indBin+2) = min(1, sum(currFA)/(datsum.nShortestITI(indCond)*obj.modelBehaviour.simulateMoreX));

                if ~isempty(obj.stim.FACutOff)
                    currFA = currRT(currOutcome == 3);
                    currFA = currFA > obj.stim.FACutOff;
                    pred(indCond,indBin+3) = min(1, sum(currFA)/currNumTrials);
                end
            ...
            for indCond = 1:size(pred,1)       % condition
                % hits and misses
                for indBin = 1:size(datsum.qm,2)+1  % quantile --> exchanged to pn as that is it... right?
...
%}
                ...
                % add false alarms
                if isnan(pred(indCond,indBin+1)), continue; end % just because there is one cell of the real and simulated data that is empty - there is no such thing as false alarm number for a "second" coherence level
                
                nShortestITI = datsum.nShortestITI(indCond)/length(obj.ppNames);
                indFA = size(datsum.qm,2)+2;
