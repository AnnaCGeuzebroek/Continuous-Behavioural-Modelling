classdef dataAnalysis < handle
    ...
    methods(Access = public)
        ...
        function getInformation(obj, preprocess, inputFolder, conditions, condNames, blockRandomized)
            ...
            if preprocess
                ...
                for indPP = 1:length(obj.ppNames)
                    ...
                    currFolder = fullfile(inputFolder, obj.ppNames{indPP}, obj.dataFolders{1});
                ...
                for indBlock = 1:length(eyeFiles)
                    ...
                    if ~exist(fullfile(obj.outputFolder, 'Eyelink data', obj.ppNames{indPP}, [obj.ppNames{indPP} '_' num2str(checkRealBlock) '_preprocessedEyelink.mat' ]), 'file')
                        ...
                        try
                            ...
                            BlockLength = BlockLength/obj.eeg.SampleRate*1000;
                        catch
                            keyboard
                        end
                        ...
                        % downsampling
                        EL.SampleRate  = BlockLength/blockEnd * 1000;
                        StepSize       = EL.SampleRate/1000;
                    ...
                    plot(EL.timeSeries(1:end-1), abs(EL.velocity).*(abs(EL.velocity) > thresholdVel), '*' )
                        ...
                        for indSac = 1:length(saccOnset)
                            ...
                            try
                                ...
                                EL.saccVel(indSac) = max(abs(diff(amplitude(saccOnset(indSac)-10:saccOffset(indSac)+10))./StepSize))*1000;
                                ...
                                try
                                    ...
                                    EL.saccVel(indSac) = max(abs(diff(amplitude(saccOnset(indSac):saccOffset(indSac)+10))./StepSize))*1000;
                                    EL.indSacc(saccOnset(indSac):saccOffset(indSac)) = 1;
                                catch
                                    ...
                                    EL.saccVel(indSac) = max(abs(diff(amplitude(saccOnset(indSac)-10:saccOffset(indSac)))./StepSize))*1000;
                                    EL.indSacc(saccOnset(indSac):saccOffset(indSac)) = 1;
                                end
                            end
                        end
        ...
        function epochingData(obj,redoEpoch)
                ...
                if isa(obj.stim.epoch ,'function_handle')
                    ...
                    for indPP = 1:length(obj.ppNames)
                        for indBlock = 1:obj.numBlocks(indPP)
                            for indTrials = 1:obj.numTrials
                                acc = acc + 1;
                                tmpEpochs(acc,:) = obj.stim.epoch(indPP,indBlock,indTrials);
                            end
                        end
                    end
                    tmpEpochs =  max(tmpEpochs,[],1);
                    if strcmpi(obj.eeg.timing, 'past')
                        obj.eeg.epoch = round((tmpEpochs(1)- max(obj.stim.lengthITI)*1000)*(obj.eeg.SampleRate/1000)):...
                            round(tmpEpochs(2)*obj.eeg.SampleRate/1000);
                    elseif strcmpi(obj.eeg.timing, 'future')
                        obj.eeg.epoch = round(tmpEpochs(1)*(obj.eeg.SampleRate/1000)):...
                            round((tmpEpochs(2) + min(obj.stim.lengthITI)*1000)*(obj.eeg.SampleRate/1000));
                    elseif isempty(obj.eeg.timing)
                        obj.eeg.epoch = round(tmpEpochs(1)*(obj.eeg.SampleRate/1000)):round(tmpEpochs(2)*obj.eeg.SampleRate/1000);
                    end
                    
                else
                    
                    if strcmpi(obj.eeg.timing, 'past')
                        obj.eeg.epoch = round((obj.stim.epoch(1)- max(obj.stim.lengthITI)*1000)*(obj.eeg.SampleRate/1000)):...
                            round(obj.stim.epoch(2)*obj.eeg.SampleRate/1000);
                    elseif strcmpi(obj.eeg.timing, 'future')
                        obj.eeg.epoch = round(obj.stim.epoch(1)*(obj.eeg.SampleRate/1000)):...
                            round((obj.stim.epoch(2) + min(obj.stim.lengthITI)*1000)*(obj.eeg.SampleRate/1000));
                    elseif isempty(obj.eeg.timing)
                        obj.eeg.epoch = round(obj.stim.epoch(1)*(obj.eeg.SampleRate/1000)):round(obj.stim.epoch(2)*obj.eeg.SampleRate/1000);
                    end
                    
                end
                obj.eeg.epochPlot = obj.eeg.epoch*1000/obj.eeg.SampleRate;
            ...
            for indPP = 1:length(obj.ppNames)
                ...
                if obj.analysisEEG
                    ...
                    currOutput = fullfile(obj.outputFolder, 'EEG data', obj.ppNames{indPP}, [obj.ppNames{indPP} '_epochedEEG_HPF' num2str(obj.eeg.HPFcutoff) obj.eeg.timing '.mat']);
                else
                    mkdir(fullfile(obj.outputFolder, 'Behavioural data', obj.ppNames{indPP}));
                    currOutput = fullfile(obj.outputFolder, 'Behavioural data', obj.ppNames{indPP}, [obj.ppNames{indPP} '_behaviour_'  obj.eeg.timing '.mat']);
                end
                ...
                if ~exist(currOutput, 'file') || redoEpoch
                    ...
                    for indBlock = 1:obj.numBlocks(indPP)
                        ...
                        if obj.analysisEEG
                            ...
                            if strcmp(obj.eeg.timing, 'future')
                                targets(end) = [];
                            end
                        else
                        ...
                        for indTarget = 1:length(targets)
                            ...
                            if obj.analysisEEG
                                ...
                                if strcmpi(obj.eeg.timing, 'past')
                                    ...
                                    indCurrEpoch = round((0-(currITI-0.5)*1000)*(obj.eeg.SampleRate/1000)):...
                                        round(currEpoch(2)*obj.eeg.SampleRate/1000);
                                elseif strcmpi(obj.eeg.timing, 'future')
                                    currITI = trialITI(indTarget+1);
                                    indCurrEpoch = round((currEpoch(1))*(obj.eeg.SampleRate/1000)):...
                                        round((currEpoch(2)+currITI*1000)*obj.eeg.SampleRate/1000);
                                elseif isempty(obj.eeg.timing)
                                    currITI = trialITI(indTarget);
                                    indCurrEpoch = round((currEpoch(1))*(obj.eeg.SampleRate/1000)):...
                                        round(currEpoch(2)*obj.eeg.SampleRate/1000);
                            ...
                            if ~isempty(nextresp)
                                for indResp = 1:length(nextresp)
                                    ...
                                    if obj.analysisEEG
                                        [~, tmpBehaviour.indReaction(targCounter,indResp)] = min(abs(obj.eeg.epochPlot - tmpBehaviour.RT(targCounter,indResp) * 1000));
                                    end
                                end
                            else
                            ...
                            % find False Alarms.
                            if strcmpi(obj.eeg.timing, 'future')
                                FAresp = find((obj.experiment{indPP}{indBlock}.RespT < obj.experiment{indPP}{indBlock}.TargOnT(indTarget)...
                                    & obj.experiment{indPP}{indBlock}.RespT > obj.experiment{indPP}{indBlock}.TargOnT(indTarget) -  obj.eeg.epochPlot(1)./1000) | ...
                                ...
                                if exist('currITI', 'var')
                                    FAresp = find(obj.experiment{indPP}{indBlock}.RespT <= obj.experiment{indPP}{indBlock}.TargOnT(indTarget)...
                                        & obj.experiment{indPP}{indBlock}.RespT > obj.experiment{indPP}{indBlock}.TargOnT(indTarget) - (currITI - (obj.stim.RTdeadLine(2)*1000 - obj.stim.duration)./1000));
                                else
                                    FAresp = [];
                                end
                            end
        ...
        function [artifact, blinks, ERP] = artifactReject(obj, ERP, EOG, RT)
            ...
            trangeTarget = obj.eeg.epochPlot >= obj.eeg.targetEpoch(1) & obj.eeg.epochPlot <= obj.eeg.targetEpoch(end);%obj.stim.RTdeadLine(2)*1000;
            ...
            for indEpoch = 1:size(ERP,3)
                ...
                trangeTarget = obj.eeg.epochPlot >= -100 & obj.eeg.epochPlot <= RT(indEpoch,1)*1000+200;
        ...
        function [rm, results, tbl] = doRANOVA(obj, plotThis, plotComb, TimeOnTask, TimeOnExperiment)
            ...
            for indPP = 1:length(obj.ppNames)
                ...
                for indCond  = 1:size(posCombination,1)
                    ...
                    reactionTimes(indPP,indCond) = nanmedian(currRT).*1000;
        ...
        function [Quality, fig] = plotERP(obj, methodUsed, plotName, Elec, BaselineCorrect, plotThis, plotComb, grouping, trangeTopo, TargetOrResponse, AverageOrSlope, MorletOrsFFT, freqRange)
            ...
            if ~exist(currTopo, 'file')
                ...
                % 2) plot average
                obj.plotAverageTopo(ERPTopo, {plotName}, [], grouping, AverageOrSlope)
                ...
                elseif iscell(Elec)
                    ...
                    obj.plotAverageTopo(ERPTopo, {plotName}, unique(indElec)', grouping, AverageOrSlope)
                else
                    obj.plotAverageTopo(ERPTopo, {plotName}, Elec, grouping, AverageOrSlope)
                ...
                if isempty(Elec)
                    ...
                    if ~exist('Elec','var')
                        ...
                        save(currTopo, 'ERPTopo',  'ERPTopoFA', 'Elec'); % re-save
                    end
                    tmpElecForPlot = Elec;
                    ...
                    % obj.plotAverageTopo(ERPTopo, {plotName}, tmpElecForPlot, grouping, AverageOrSlope)
                end
                
                % 3) plot average to check location of electrodes.
                %                 obj.plotAverageTopo(ERPTopo, {plotName}, tmpElecForPlot, grouping, AverageOrSlope)
                % obj.plotAverageTopo(ERPTopoFA, {strcat(plotName, 'FA')}, tmpElecForPlot, grouping, AverageOrSlope)
            end
            ...
            if ~exist(currOutput, 'file')
                ...
                for indPP = 1:length(obj.ppNames)
                    ...
                    elseif methodUsed == 2
                        ...
                        % limited the electrodes to the centro-partial
                        % electrodes. These were determined by the average
                        % topoplot
                        %{
                    y = quantile(evalSNR,0.75)
                    indBestElec = find(evalSNR >= y)
                    indElec(indPP, 1:length(indBestElec)) = Elec(indBestElec);
                    ERPWhole(1,:,1:size(ERP,3),indPP) = nanmean(ERP(indElec(indPP, 1:length(indBestElec)),:,:),1);
                        %}
                    ...
                    elseif methodUsed == 3
                        ...
                        if strcmp(obj.eeg.timing, 'future')
                            keyboard
                            ERP2 = ERP(:,obj.eeg.epochPlot  >= 1000 &  obj.eeg.epochPlot  < 1000 + size(ERP1,2)*(1000/obj.eeg.SampleRate),:);
                        else
                            tmpWindow = obj.eeg.epochPlot  >= -(min(obj.stim.lengthITI)*1000-1000) &  obj.eeg.epochPlot  < -(min(obj.stim.lengthITI)*1000-1000)+size(ERP1,2)*(1000/obj.eeg.SampleRate);
                            ERP2 = ERP(:, tmpWindow,:);
                        end
                        
                        % 2.1) artificat rejection.
                        %                     ERP1(:, :, art2) = nan;
                        %                     ERP2(:, :, art2) = nan;
                        ...
                        % figure, plot(obj.eeg.responseEpoch, nanmean(nanmean(ERP1,1),3))
                        % hold on, plot(obj.eeg.responseEpoch, nanmean(nanmean(ERP2,1),3))
            ...
            for indPP = 1:length(obj.ppNames)
                ...
                if  obj.DetectOrDisc && size(plotTarget, 1) > 1 % (plotThis == 2 || plotThis == 6) &&
                    ...
                    %                     tmpplotTarget    = repmat(nanmean(plotTarget(:,:,:,indPP),1),2,1,1);
                    %                     tmpplotResponse  = repmat(nanmean(plotResponse(:,:,:,:,indPP),1),2,1,1);
                    %
                    %                     tmpplotTarget(1,) = plotTarget(1,:,obj.behaviour{indPP}.Reaction == 1,indPP);
                    %                     tmpplotTarget(1,:,obj.behaviour{indPP}.Reaction == 2) = plotTarget(2,:,obj.behaviour{indPP}.Reaction == 2,indPP);
                    %
                    %                     tmpplotTarget(2,:,obj.behaviour{indPP}.Reaction == 1) = plotTarget(2,:,obj.behaviour{indPP}.Reaction == 1,indPP);
                    %                     tmpplotTarget(2,:,obj.behaviour{indPP}.Reaction == 2) = plotTarget(1,:,obj.behaviour{indPP}.Reaction == 2,indPP);
                    %
                    %                     tmpplotResponse(1,:,obj.behaviour{indPP}.Reaction == 1) = plotResponse(1,:,obj.behaviour{indPP}.Reaction == 1,:,indPP);
                    %                     tmpplotResponse(1,:,obj.behaviour{indPP}.Reaction == 2) = plotResponse(2,:,obj.behaviour{indPP}.Reaction == 2,:,indPP);
                    %
                    %                     tmpplotResponse(2,:,obj.behaviour{indPP}.Reaction == 1) = plotResponse(2,:,obj.behaviour{indPP}.Reaction == 1,:,indPP);
                    %                     tmpplotResponse(2,:,obj.behaviour{indPP}.Reaction == 2) = plotResponse(1,:,obj.behaviour{indPP}.Reaction == 2,:,indPP);
                    %
                    %                     plotTarget(1,:,:, indPP)     = tmpplotTarget(1,:,:);
                    %                     plotResponse(1,:,:,:, indPP) = tmpplotResponse(1,:,:,:);
                    %                 elseif obj.DetectOrDisc && size(plotTarget, 1) > 1
            ...
            if  plotThis ~= 1
                ...
                if  nanmean(plotTarget(:)) > 10 & MorletOrsFFT == 2
                    
                    windowSize = 4 * (1/obj.stim.freqSSVEP)*1000;
                    L    = floor(windowSize/(1000/Fs)); %sum(obj.eeg.epochPlot >= 0 & obj.eeg.epochPlot <= windowSize);    % samples %f  = 0:dF:Fs-dF + (dF/2)*mod(L,2);      % hertz  %dF = Fs/L;               % hertz per sample
                    
                    plotTarget      = plotTarget ./L;
                    plotResponse    = plotResponse ./L;
                    plotFA          = plotFA ./L;
                elseif  nanmean(plotTarget(:)) > 10 & strfind(plotName, 'SSVEP')
                ...
                %                 if ~any(plotComb == 0)
                %                 	obj.plotAverageSurface(plotTarget, plotName, grouping, plotComb);
                %                 end
            end
        ...
        function [donetbl, waveform, plotData] = getWaveformParameters(obj, methodUsed, plotName, qps, BaselineCorrect, plotThis, plotComb, grouping, trangeTopo, TargetOrResponse, negOrPos,PeakMeanOrMax, amplitudeAxis)
            % check for parameters.
            if ~exist('TargetOrResponse', 'var');   TargetOrResponse = 2;  end  % standard on response-locked
            ...
            % preset folders
            saveFolder = fullfile(obj.outputFolder, 'EEG data', 'groupAverage', plotName, ['RTBINS' num2str(length(qps)) '\']);
            ...
            if ~isempty(qps)
                within.Name = {'RTs' within.Name{:}};
                within.Par  = [1 within.Par];
                within.Var  = {1:size(qps)+1 within.Var{:}};
                
                tmpCondition = [];
                for indCond = 1:within.numCond
                    tmpCondition = [tmpCondition;  (1:length(qps)+2)', repmat(within.Condition(indCond,:),length(qps)+2,1)];
                end
            ...
            if ~exist(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse) '_' num2str(BaselineCorrect) '.mat']), 'file')
                ...
                for indPP = 1:length(obj.ppNames)
                    ...
                    if nanmean(obj.behaviour{indPP}.artifacts & obj.behaviour{indPP}.blinks) >= 0.3
                        ...
                        currtblOnset        = nan(size(goodTrials));
                        ...
                        for indCond = 1:size(posCombination,1)
                            ...
                            if ~isempty(tmpCond)
                                ...
                                signalPeak  = squeeze(Signal(:,:,tmpCond,indPP)); signalPeak  = sgolayfilt(signalPeak,1,51,[],1);
                                ...
                                elseif PeakMeanOrMax(1) == 2
                                    ...
                                    currtblPeakLatency(tmpCond) = tmpLatency./1000;
                                end
                                ...
                                for indEpoch = 1:length(tmpRT)
                                    if ~isnan(tmpRT(indEpoch))
                                        indSlope = obj.eeg.responseEpoch> tmpLatency(indEpoch)-(tmpRT(indEpoch)*1000) &  obj.eeg.responseEpoch < tmpLatency(indEpoch); %
                                ...
                                % for higher signal to noise ratio (issues
                                % with low level as it kicks out more).
                                %                                 if negOrPos == 1
                                %                                 	signalPeak(:,tmpSlope >= 0 | tmpAmplitude' >= 0) = NaN;
                                %                                 elseif negOrPos == 2
                                %                                 	signalPeak(:,tmpSlope <= 0 | tmpAmplitude' <= 0) = NaN;
                                %                                 end
                                ...
                                for indQps = 1:length(qps)+2
                                    ...
                                    elseif PeakMeanOrMax(1) == 1 && ~isempty(currQps) && ~isnan(waveform.RT(indPP,acc))
                                        ...
                                        waveform.peak(indPP, acc)        = nanmedian(nanmean(signalPeak(PeakArea,currQps),1),2);
                                    ...
                                    %{
                                    if isnan(waveform.peak(indPP, acc))
                                        signalOnset = 0;
                                    else
                                        if negOrPos == 1
                                            [~, pValues] = permtest(currSignal, 0, 10000, 0.05, 'left');
                                        elseif negOrPos == 2
                                            [~, pValues] = permtest(currSignal, 0, 10000, 0.05, 'right');
                                        end
                                        
                                        hypo =  pValues < 0.05;
                                        
                                        if any( diff(find(hypo)) > 10)
                                            tmpHypo = diff(hypo);
                                            times = (find(tmpHypo > 0));
                                            
                                            times((timeSeries{2}(times) - 100) < 0 | times+10 > length(hypo) |  timeSeries{2}(times) > waveform.peakLatency(indPP, acc)) = [];
                                            checkduration = [];
                                            for indTime = 1:length(times)
                                                checkduration(indTime) =  sum(hypo(times(indTime)+1:times(indTime)+10));
                                            end
                                            [~, indFind]= min( waveform.peakLatency(indPP, acc) - timeSeries{2}(times( checkduration > 9)));
                                            signalOnset =  timeSeries{2}(times( indFind)) - 100;
                                            
                                            if isempty(signalOnset)
                                                signalOnset = 0;
                                            end
                                            
                                        elseif sum(diff(find(hypo))) > 10
                                            signalOnset  = timeSeries{2}(find(hypo,1)) - 100;
                                        else
                                            tmpSignal =  nanmean(currSignal(:,timeSeries{2} < waveform.peakLatency(indPP,acc)), 1);
                                            tmpTimeSeries = fliplr(timeSeries{2}(timeSeries{2} < waveform.peakLatency(indPP,acc)));
                                            
                                            if negOrPos == 2
                                                signalOnset = tmpTimeSeries(find(fliplr(tmpSignal <= 0),1));
                                            elseif negOrPos == 1
                                                signalOnset = tmpTimeSeries(find(fliplr(tmpSignal >= 0),1));
                                            end
                                    ...
                                    if ~isempty(currSignal) | ~all(isnan(currSignal))
                                        %{
                                        x = timeSeries{2}(timeSeries{2} > signalOnset  &  timeSeries{2} < waveform.peakLatency(indPP, acc));
                                        y = currSignal (:, timeSeries{2} > signalOnset &  timeSeries{2} < waveform.peakLatency(indPP, acc));
                                        keyboard
                                        clear Slope interval
                                        for indEpoch = 1:size(y,1)
                                            % Fit line to data using polyfit
                                            c = polyfit(x,y(indEpoch,:),1);
                                            % Display evaluated equation y = m*x + b
                                            %disp(['Equation is y = ' num2str(c(1)) '*x + ' num2str(c(2))])
                                            % Evaluate fit equation using polyval
                                            Slope(indEpoch,:) = c(1);
                                            interval(indEpoch,:) = c(2);
                                            %y_est = polyval(c,x);
                                        end
                                     
                                        keyboard
                                        tbl.Slopes{indPP}(tmpCond(currQps)) = Slope;
                                        %}
                                        
                                        waveform.slopes(indPP, acc)   = nanmean(tmpSlope(currQps),1);
                                        % waveform.onset(indPP, acc)  = signalOnset;
                                        waveform.numTrials(indPP, acc)	= sum(currQps);
                                        
                                    else
                                        waveform.slopes(indPP, acc)   = nan;
                                        waveform.interval(indPP, acc) = nan;
                                        % waveform.onset(indPP, acc)    = nan;
                                    end
                        ...
                        % tbl.RTQuar       = [tbl.RTQuar tmptblRTQuar];
                ...
                save(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(BaselineCorrect) '.mat']), 'Signal', 'timeSeries', 'slopeArea', 'tbl', 'waveform')
            else
                load(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(BaselineCorrect) '.mat']), 'Signal', 'timeSeries', 'slopeArea', 'tbl', 'waveform')
            end
            
            %{
            colorHere = flipud(brewermap(5, 'greys'));
            % first plot unadjusted to get an quick overview
            for indPP = 1:length(obj.ppNames)
                if ~exist(fullfile(indivFolder, 'Automatic', [obj.ppNames{indPP} '_SlopesAndPeak' num2str(TargetOrResponse(1)) '.png']), 'file')
                    if ~isfolder(fullfile(indivFolder, 'Automatic')), mkdir(fullfile(indivFolder, 'Automatic')); end
                    if nanmean(obj.behaviour{indPP}.artifacts & obj.behaviour{indPP}.blinks) >= 0.3
                        % 4) go through given conditions and sort, bin and
                        % average signals
                        goodTrials      = obj.behaviour{indPP}.artifacts' & obj.behaviour{indPP}.blinks';
                        ...
                        for indCond = 1:size(posCombination,1)
                            ...
                            if ~isempty(tmpCond)
                                ...
                                % extract the aligned ERPs for Amplitude
                                % and slope extraction
                                signalSlope = squeeze(Signal(:,:,tmpCond,indPP));
                                
                                % sliding window of 100 ms over the
                                % amplitude and slope
                                signalSlope = sgolayfilt(signalSlope,1,51,[],1);
                                
                                tmpRTqps =  tbl.RTQuar{indPP}(tmpCond);
                                
                                subplot(2,2, indCond), hold on
                                for indQps = 1:length(qps)+2
                                    currQps  = tmpRTqps == indQps;
                                    currSignal = signalSlope(slopeArea, currQps);
                                    
                                    plot(timeSeries{2}, nanmean(currSignal,2), 'LineWidth', 2, 'Color', colorHere(indQps,:))
                                    plot(waveform.peakLatency(indPP, acc), waveform.peak(indPP, acc), 'r*')
                                    % plot(waveform.onset(indPP, acc), 0, 'r*')
                                    
                                    x = timeSeries{2}( timeSeries{2} >  waveform.onset(indPP, acc)  &  timeSeries{2} < waveform.peakLatency(indPP, acc));
                                    
                                    if ~isnan(waveform.slopes(indPP, acc))
                                        y_est = polyval([ waveform.slopes(indPP, acc) waveform.interval(indPP, acc)], x);
                    ...
                    plotSave(gca,  [obj.ppNames{indPP} '_SlopesAndPeak' num2str(TargetOrResponse(1)) '.png'], fullfile(indivFolder, 'Automatic/'), [10 10]);
                else
                    clc
                    fprintf('Automatic plots can be found in %s\n', 'Automatic')
                end
            end
            %}
            %close all
            ...
            %% handmatig aanpassen
            % reset the trial-to-trial stuff
            %{
            if TargetOrResponse == 1
                set(0,'DefaultFigureWindowStyle','docked')
                for  indPP = 1:length(obj.ppNames)
                    if exist(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(indPP) '.mat']), 'file')
                        load(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(indPP) '.mat']))
                    else
                        if nanmean(obj.behaviour{indPP}.artifacts & obj.behaviour{indPP}.blinks) >= 0.3
                            % 4) go through given conditions and sort, bin and
                            % average signals
                            ...
                            for indCond =  1:size(posCombination,1)
                                ...
                                if ~isempty(tmpCond)
                                    ...
                                    subplot(2,2,indCond); hold on
                                    indQps = 1; clear curr*
                                    while indQps <= length(qps)+2
                                        
                                        if ~(isnan(waveform.RT(indPP,acc)) && TargetOrResponse == 2)
                                            currQps = tmpRTqps == indQps;
                                            if sum(currQps) > 0
                                                ...
                                                plot(waveform.onset(indPP, acc), 0, 'r*')
                                                if TargetOrResponse == 1
                                                    line([waveform.RT(indPP, acc).*1000 waveform.RT(indPP, acc).*1000], [5 -5])
                                                end
                                                
                                                drawnow
                                                
                                                noPeak = [];
                                                fprintf('Is there peak Correct?\n');
                                                WaitSecs(0.2);
                                                [~, keyCode] =  KbWait;
                                                
                                                clear ind
                                                if strcmp(KbName(keyCode), 'n')
                                                    [noPeak, ~] = ginput(1);
                                                    if noPeak == 0
                                                        waveform.peak(indPP, acc) = nan;
                                                        waveform.peakLatency(indPP, acc) = timeSeries{2}(end);
                                                    else
                                                        waveform.peakLatency(indPP, acc) = noPeak;
                                                        [~, ind] = min(abs(timeSeries{2} - noPeak));
                                                        tbl.Peak{indPP}(tmpCond(currQps)) = nanmean(currSignal(timeSeries{2} > waveform.peakLatency(indPP, acc) - 50 & timeSeries{2} < waveform.peakLatency(indPP, acc) + 50,:),1);
                                                        waveform.peak(indPP, acc) = nanmean(currSignal(ind, :),2);
                                                        plot(waveform.peakLatency(indPP, acc), waveform.peak(indPP, acc), 'k*')
                                                    end
                                                end
                                                
                                                
                                                selectedTime =  timeSeries{2}(timeSeries{2} > waveform.peakLatency(indPP, acc)  - 50 & timeSeries{2} < waveform.peakLatency(indPP, acc) + 50);
                                                
                                                if negOrPos == 1
                                                    [~, ind] =  min(currSignal(timeSeries{2} > waveform.peakLatency(indPP, acc) - 50 & timeSeries{2} < waveform.peakLatency(indPP, acc) + 50,:));
                                                elseif negOrPos == 2
                                                    [~, ind] =  max(currSignal(timeSeries{2} > waveform.peakLatency(indPP, acc) - 50 & timeSeries{2} < waveform.peakLatency(indPP, acc) + 50,:));
                                                end
                                                
                                                if ~isempty(ind)
                                                    tbl.PeakLatency{indPP}(tmpCond(currQps)) =  selectedTime(ind);
                                                end
                                                
                                                onsetCorrect = [];
                                                fprintf('Is the onset correct?\n');
                                                WaitSecs(0.2);
                                                [~, keyCode] = KbWait;
                                                
                                                if strcmp(KbName(keyCode), 'n')
                                                    [onsetCorrect, y] = ginput(1);
                                                    waveform.onset(indPP, acc)  = onsetCorrect;
                                                    plot(waveform.onset(indPP, acc), y, 'k*')
                                                end
                                                
                                                x = timeSeries{2}( timeSeries{2} >  waveform.onset(indPP, acc)  &  timeSeries{2} < waveform.peakLatency(indPP, acc));
                                                
                                                if ~isempty(onsetCorrect) || ~isempty(noPeak)
                                                    y = currSignal (timeSeries{2} >  waveform.onset(indPP, acc)   &  timeSeries{2} < waveform.peakLatency(indPP, acc),:)';
                                                    ...
                                                    waveform.slopes(indPP, acc)   = nanmean(Slope,1);
                                                    waveform.interval(indPP, acc) = nanmean(interval,1);
                                            ...
                                            clc
                                            WaitSecs(0.2);
                                            fprintf('Are you happy with the fit?\n');
                                            [~, keyCode] =  KbWait;
                                            if ~strcmp(KbName(keyCode), 'n')
                                                indQps = indQps + 1;
                                                acc = acc + 1;
                        ...
                        plotSave(gca,  [obj.ppNames{indPP} '_SlopesAndPeak' num2str(TargetOrResponse(1)) '.png'], fullfile(indivFolder, 'Manual/'), [10 10]);
                        save(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(indPP) '.mat']))
                        close all
                    end
                end
            ...
            %{
                ...
                for indPP = 1:length(obj.ppNames)
                    subplot(221);  hold on;
                    currCond = tbl.condition(:,1) == 1 & tbl.coherence == 25 & tbl.ppNames == indPP;
                    scatter(tbl.RT(currCond), tbl.Peak(currCond), [], obj.figLayOut.colours(1,:));
                    subplot(222); hold on
                    currCond = tbl.condition(:,1) == 2 & tbl.coherence == 25 & tbl.ppNames == indPP;
                    scatter(tbl.RT(currCond), tbl.Peak(currCond), [], obj.figLayOut.colours(2,:));
                    subplot(223); hold on
                    currCond = tbl.condition(:,1) == 1 & tbl.coherence == 70 & tbl.ppNames == indPP;
                    scatter(tbl.RT(currCond), tbl.Peak(currCond), [], obj.figLayOut.colours(3,:));
                    subplot(224); hold on
                    currCond = tbl.condition(:,1) == 2 & tbl.coherence == 70 & tbl.ppNames == indPP;
                    scatter(tbl.RT(currCond), tbl.Peak(currCond), [], obj.figLayOut.colours(4,:));
                end
                
                for indSub = 1:4
                    subplot(2,2,indSub)
                    currLine = lsline; %currLine.Color = parameters.figLayOut.colours(indSub,:); currLine.LineWidth = parameters.figLayOut.lineWidth;
                end
            ...
            %             for indCond = 1:length(plotComb)
            %                 eval(sprintf('tbl.%s = categorical(tbl.%s);',  within.Name{indCond},  within.Name{indCond}));
            %             end
            ...
            %             if strfind(plotName, 'ERD') ~= 0 & any(waveform.peak(:) < 0 )
            %                waveform.peak = waveform.peak .*-1;
            %             end
            
            if plotThis ~= 0
                %                 waveform.peak = waveform.slopes;
                if isempty(qps)
                    waveform.RT(:,2:2:end) = [];
                    waveform.peak(:,2:2:end) =[];
                    waveform.slopes(:,2:2:end) =[];
                else
                    waveform.RT(:,within.Condition(:,1) == (length(qps)+ 2)) = [];
                    waveform.peak(:,within.Condition(:,1) == (length(qps)+ 2)) =[];
                    waveform.slopes(:,within.Condition(:,1) == (length(qps)+ 2)) =[];
                    within.Table(within.Condition(:,1) == (length(qps)+ 2),:) = [];
                end
                ...
                %{
            signalTable = array2table(waveform.peak);
            rtTable = array2table(waveform.RT);
            
            if sum(~within.Par)
                eval(sprintf('signalTable.%s = categorical(betweenTable'');', betweenName{1}));
                rm.Peak = fitrm(signalTable, sprintf('Var1-Var%i ~ %s', size(waveform.peak,2), betweenName{1}), 'WithinDesign', withinTablePeak);
                rm.RT = fitrm(rtTable, sprintf('Var1-Var%i ~ %s', size(waveform.RT,2), betweenName{1}), 'WithinDesign', withinTablePeak);
                plotData = margmean(rm.Peak, {withinName betweenName{1}});
                plotRT = margmean(rm.RT, {withinName betweenName{1}});
            else
                rm.Peak = fitrm(signalTable, sprintf('Var1-Var%i ~ 1', size(waveform.peak,2)), 'WithinDesign', within.Table);
                rm.RT = fitrm(rtTable, sprintf('Var1-Var%i ~ 1', size(waveform.RT,2)), 'WithinDesign', within.Table);
                plotData = margmean(rm.Peak, within.Name);
                plotRT = margmean(rm.RT, within.Name);
            end
                %}
                if any(strcmp(within.Name, 'RTs'))
                    posCombination = unique(within.Condition(:,2:end), 'row');
                else
                ...
                [rm.RT, results.RT] = preformRANOVA(obj, fileID, 'RT Amplitude', strCond,  waveform.RT.*1000, between, within, modelFun);
                [rm.Slopes, results.Slopes] = preformRANOVA(obj, fileID, 'Slope', strCond,  waveform.slopes, between, within, modelFun);
                ...
                if isempty(qps)
                    ...
                    %                 ylim([20 45]);  yticks(25:5:45)
                    %
                    %                 ylim([50 150]); yticks(75:25:200)
                    %                 ylabel({' ' ' '})
                    set(gca,'FontSize', obj.figLayOut.letterSize);
                    set(gca,'FontName', obj.figLayOut.letterType);
                    
                    plotSave(gca, ['amplitude' plotName '.png'], hereFigFolder, [4.173 2.5]);%[4.3 obj.figLayOut.saveDim(2)/3]);
                    %}
                else
                    ...
                    for indCond = 1: within.numCond
                        currPlot   = all( table2array ( plotData(:,2:size(posCombination,2)+1)) == categorical(posCombination(indCond,:)), 2);
                        ...
                        legendThis(indCond) = errorbar(averageRT(indCond,:), averageSignal(indCond,:), stdSignal(indCond,:));%, 'CapSize',0.1);
                    ...
                    figure(fig2)
                    for indCond = 1: within.numCond
                        currPlot   = all( table2array ( plotSlopes(:,2:size(posCombination,2)+1)) == categorical(posCombination(indCond,:)), 2);
                        averageRT(indCond, :)     = plotRT.Mean(currPlot);
            ...
            if ~exist(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse) '_' num2str(BaselineCorrect) '.mat']), 'file')
                ...
                if nanmean(ERPTarget(:)) > 10 && contains(plotName, 'Beta')
                    windowSize = 4 * (1/obj.stim.freqSSVEP)*1000;
                    L    = floor(windowSize/(1000/obj.eeg.SampleRate)); %sum(obj.eeg.epochPlot >= 0 & obj.eeg.epochPlot <= windowSize);    % samples %f  = 0:dF:Fs-dF + (dF/2)*mod(L,2);      % hertz  %dF = Fs/L;               % hertz per sample
                    
                    ERPTarget      = ERPTarget ./L;
                    ERPResponse    = ERPResponse ./L;
                end
                
                % 1) get max amplitude according at response
                if TargetOrResponse == 1
                    PeakArea      = obj.eeg.targetEpoch > trangeTopo(1,1) & obj.eeg.targetEpoch < trangeTopo(1,2);
                    timeSeries    = obj.eeg.targetEpoch(PeakArea);
                    
                    if size(ERPTarget, 1) > 1
                        ERPTarget(1,:,:,:) = nanmean(ERPTarget,1);
                        ERPTarget(2,:,:,:) = [];
                    end
                    Signal = ERPTarget;
                ...
                for indPP = 1:length(obj.ppNames)
                    ...
                    if nanmean(obj.behaviour{indPP}.artifacts & obj.behaviour{indPP}.blinks) >= 0.3
                        ...
                        currtblRT       = nan(size(goodTrials));
                        currtblPeak 	= nan(size(goodTrials));
                        currtblSlope	= nan(size(goodTrials));
                        currtblqpsRT	= nan(size(goodTrials));
                        acc = 1;
                        ...
                        for indCond = 1:size(posCombination,1)
                            ...
                            if ~isempty(tmpCond)
                                ...
                                if isempty(qps)
                                    tmpRTqps = discretize(tmpRT, [0 max(tmpRT)], 'IncludedEdge','right');
                                    tmpRTqps(isnan(tmpRTqps))   = 1;%length(qps)+2; % add the misses
                                else
                                    tmpRTqps = discretize(tmpRT, [0 quantile(tmpRT,qps)-0.5/60 max(tmpRT)], 'IncludedEdge','right');
                                    tmpRTqps(isnan(tmpRTqps))   = length(qps)+2; % add the misses
                                end
                                ...
                                currtblRT(tmpCond)      = tmpRT;
                                currtblqpsRT(tmpCond)	= tmpRTqps;

                                % extract the aligned ERPs for Amplitude
                                % and slope extraction
                                signalPeak  = squeeze(Signal(:,:,tmpCond,indPP));
                                signalPeak  = sgolayfilt(signalPeak,1,51,[],1);
                                ...
                                elseif PeakMeanOrMax(1) == 2
                                    if negOrPos == 1
                                        tmpAmplitude = min(signalPeak(PeakArea,:));
                                    elseif negOrPos == 2
                                        tmpAmplitude = max(signalPeak(PeakArea,:));
                                    end
                                end
                                ...
                                for indEpoch = 1:length(tmpRT)
                                    if ~isnan(tmpRT(indEpoch))
                                        indSlope = obj.eeg.responseEpoch > trangeTopo(2,1)   &  obj.eeg.responseEpoch < trangeTopo(2,2);
                                        x = obj.eeg.responseEpoch(indSlope);
                                        y = signalPeak(indSlope, indEpoch)';
                                ...
                                for indQps = 1:length(qps)+2
                                    ...
                                    if sum(currQps) == 0 || isnan(waveform.RT(indPP,acc))
                                        waveform.peak(indPP, acc) = NaN;
                                    elseif PeakMeanOrMax(1) == 1 && ~isempty(currQps) && ~isnan(waveform.RT(indPP,acc))
                                        waveform.peak(indPP, acc) = nanmean(nanmedian(signalPeak(PeakArea,currQps),1),2);
                                    elseif PeakMeanOrMax(1) == 2
                                        % 2.2) reshape rt into vector and sort and allRT to get line.
                                        tmpMaxSignal = nanmean(signalPeak(PeakArea, currQps),2);
                                        
                                        if negOrPos == 1
                                            peak = min(tmpMaxSignal);
                                        elseif negOrPos == 2
                                            peak = max(tmpMaxSignal);
                                        end
                                        waveform.peak(indPP, acc) = peak;
                                    end
                                    
                                    if ~isnan(waveform.RT(indPP,acc))
                                        indSlope = obj.eeg.responseEpoch > trangeTopo(2,1)  &  obj.eeg.responseEpoch < trangeTopo(2,2);
                                        x = obj.eeg.responseEpoch(indSlope);
                                        y = nanmean(signalPeak(indSlope, currQps),2)';
                                        
                                        % Fit line to data using polyfit
                                        c = polyfit(x,y,1);
                                        % Evaluate fit equation using polyval
                                        waveform.Slope(indPP, acc) = c(1);
                                        waveform.Intersect(indPP, acc) = c(2);
                                    else
                                        waveform.Slope(indPP, acc) = nan;
                                        waveform.Intersect(indPP, acc) = nan;
                                    end
                                    acc = acc + 1;
                                end
                            end
                        ...
                        tbl.ppNames      = [tbl.ppNames;  repmat(indPP, size(currtblRT))];
                        tbl.RT           = [tbl.RT;  nanzscore(currtblRT)];
                        tbl.qpsRT        = [tbl.qpsRT;  currtblqpsRT];
                        tbl.Peak         = [tbl.Peak; (currtblPeak)];
                        tbl.Slope        = [tbl.Slope; (currtblSlope)];
                ...
                save(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(BaselineCorrect) '.mat']), 'Signal', 'timeSeries', 'tbl', 'waveform')
            else
                load(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(BaselineCorrect) '.mat']), 'Signal', 'timeSeries', 'tbl', 'waveform')
            end
            
            %
            colorHere = flipud(brewermap(length(qps)+1, 'greys'));
                    ...
                    if nanmean(obj.behaviour{indPP}.artifacts & obj.behaviour{indPP}.blinks) >= 0.3
                        ...
                        for indCond = 1:size(posCombination,1)
                            ...
                            if ~isempty(tmpCond)
                                ...
                                tmpRTqps =  tbl.qpsRT(tbl.ppNames == indPP);
                                tmpRTqps =  tbl.qpsRT(tmpCond);
                                
                                subplot(2,2, indCond), hold on
                                for indQps = 1:length(qps)+1
                                    ...
                                    plot(x,y , 'LineWidth', 2, 'Color', colorHere(indQps,:))
                                    % plot(waveform.peakLatency(indPP, acc), waveform.peak(indPP, acc), 'r*')
                                    % plot(waveform.onset(indPP, acc), 0, 'r*')
                                    
                                    if ~isnan(waveform.Slope(indPP, acc))
                                        y_est = polyval([ waveform.Slope(indPP, acc) waveform.Intersect(indPP, acc)], x);
                                        % Add trend line to plot
                                        plot(x,y_est,'r--','LineWidth',2)
                                    end
                                    keyboard
                    ...
                    if exist(fullfile(saveFolder, ['Parameters ' num2str(plotComb) num2str(TargetOrResponse(1)) '_' num2str(indPP) '.mat']), 'file')
                        ...
                        if nanmean(obj.behaviour{indPP}.artifacts & obj.behaviour{indPP}.blinks) >= 0.3
                            ...
                            for indCond =  1:size(posCombination,1)
                                ...
                                clear tmp*
                                tmpCond = find(sum(VartrialMatrix == posCombination(indCond,:),2) == size(posCombination, 2) &...
                                    goodTrials);
                                ...
                                if ~isempty(tmpCond)
                                    ...
                                    tmpRTqps = tbl.RTQuar(tmpCond, indPP);
                                    ...
                                    while indQps <= length(qps)+2
                                        ...
                                        if ~(isnan(waveform.RT(indPP,acc)) && TargetOrResponse == 2)
                                            ...
                                            if sum(currQps) > 0
                                                ...
                                                if ~isempty(ind)
                                                    tbl.PeakLatency(tmpCond(currQps)) =  selectedTime(ind);
                                                end
            ...
            for indCond = 1:length(plotComb)
                eval(sprintf('tbl.%s = (tbl.%s);',  within.Name{indCond},  within.Name{indCond}));
            end
            ...
            %}
                    ...
                    plotSave(gca, ['amplitude' plotName '.png'], hereFigFolder, [5 5]);%[4.173 2.5]);%[4.3 obj.figLayOut.saveDim(2)/3]);
                else
            ...
            maxRT = max(plotRT.Mean);
            maxRT = 0.5* ceil((maxRT/0.5));

            %{
            for indCond = 1:length(posCombination)
                if any(plotData.RTs(all( table2array ( plotData(:,1:size(posCombination,2))) == posCombination(indCond,:), 2)) == 4)
                    tmp = errorbar(maxRT + disPlot(indCond), averageSignal(indCond,4), stdSignal(indCond,4), '.');%, 'CapSize',0);
                    tmp.Color = obj.figLayOut.colours(indCond,:);
                    tmp.LineWidth = 1;
                    tmp.LineStyle = obj.figLayOut.lineType{indCond};
                end
            end
            %}
            ...
            line([0 0],  [yaxis(1) yaxis(2)], 'Color', 'k', 'LineWidth', 1.5)
            
            ylabel([{plotName} {' Peak (\muV/m^2)'}])
            xstuff = [0:500:maxRT*1000]./1000;
            ...
            plotSave(gca, ['RTvsWavePlot' plotName num2str(TargetOrResponse) 'Peak.png'], hereFigFolder, [4.3 obj.figLayOut.saveDim(2)/3]);
            %}
            %% run the ranovas on the experiment.
            %{
            % ------------- peak latency ---------------------------------
            if TargetOrResponse(1) == 1
                % write away the descriptives statistics
                % remove inter-subject variabiltiy
                waveform.peakLatency(:,noEnoughPP)  = [];
                
                indMean   = repmat(nanmean(  waveform.peakLatency,2), 1, size(  waveform.peakLatency,2));
                grandMean = nanmean(  waveform.peakLatency(:));
                
                waveform.peakLatency =  waveform.peakLatency - indMean + grandMean;
                signalTable = array2table(waveform.peakLatency);
                
                if sum(~within.Par)
                    eval(sprintf('signalTable.%s = categorical(betweenTable'');', betweenName{1}));
                    rm.peakLatency = fitrm(signalTable, sprintf('Var1-Var%i ~ %s', size(waveform.peakLatency,2), betweenName{1}), 'WithinDesign', within.Table);
                    plotData = margmean(rm.peakLatency, {within.Name between.Name{1}});
                else
                    rm.peakLatency = fitrm(signalTable, sprintf('Var1-Var%i ~ 1', size(waveform.peakLatency,2)), 'WithinDesign', within.Table);
                    plotData = margmean(rm.peakLatency, within.Name);
                end
                ...
                clear averageSignal stdSignal averageRT
                for indCond = 1:length(posCombination)
                    currPlot   = all( table2array ( plotData(:,(1:size(posCombination,2))+1)) == posCombination(indCond,:), 2);
                    averageRT(indCond, :)     = plotRT.Mean(currPlot);
                    
                    averageSignal(indCond, :) = plotData.Mean(currPlot);
                    stdSignal(indCond, :)     = plotData.StdErr(currPlot);
                    
                    legendThis(indCond) = errorbar(averageRT(indCond,1:3), averageSignal(indCond,1:3), stdSignal(indCond,1:3), 'CapSize',0);
            ...
            %}
                xlim([0 obj.eeg.targetEpoch(end)]./1000)
                
                figInfo = gca;
                xstuff = [0:500:maxRT*1000]./1000;
                
                ylim([xstuff(1) xstuff(end)+0.150].*1000)
                line([0 0],  [0  (xstuff(end)+0.15).*1000], 'Color', 'k', 'LineWidth', 1.5)
                
                ylabel([{plotName} {' Peak latency (ms)'}])
                yticks([xstuff].*1000)
                yticklabels(xstuff)
                ...
                plotSave(gca, ['RTvsWavePlot' plotName  num2str(TargetOrResponse) 'Peaklatency.png'], hereFigFolder, [4.3 obj.figLayOut.saveDim(2)/2]);
            end
            %% ------------- Slope ---------------------------------
            % write away the descriptives statistics
            waveform.slopes(:,noEnoughPP)  = [];
            ...
            waveform.slopes =  waveform.slopes - indMean + grandMean;
            signalTable = array2table(waveform.slopes);
            
            if sum(~within.Par)
                eval(sprintf('signalTable.%s = categorical(betweenTable'');', betweenName{1}));
                rm.slopes = fitrm(signalTable, sprintf('Var1-Var%i ~ %s', size(waveform.slopes,2), betweenName{1}), 'WithinDesign', within.Table);
                plotData = margmean(rm.slopes, {within.Name between.Name{1}});
            else
                rm.slopes = fitrm(signalTable, sprintf('Var1-Var%i ~ 1', size(waveform.slopes,2)), 'WithinDesign', within.Table);
                plotData = margmean(rm.slopes, within.Name);
            end
            ...
            clear averageSignal stdSignal averageRT
            for indCond = 1:length(posCombination)
                currPlot   = all( table2array ( plotData(:,1:size(posCombination,2))) == posCombination(indCond,:), 2);
                averageRT(indCond, :)     = plotRT.Mean(currPlot);
...
if TargetOrResponse == 1
                xlim([xstuff(1) maxRT+0.15])
                xticks([xstuff maxRT])
                xticklabels({xstuff; 'Misses' });
            else
            %}
            ...
            ylabel([{plotName} {' Slope (\muV/m^2/Ts)'}])
            
            set(gca,'FontSize', obj.figLayOut.letterSize);
            set(gca,'FontName', obj.figLayOut.letterType);
            
            plotSave(gca, ['RTvsWavePlot' plotName num2str(TargetOrResponse) 'slopes.png'], hereFigFolder, [4.3 obj.figLayOut.saveDim(2)/2]);
            set(0,'DefaultFigureWindowStyle','normal')
            %}
        ...
        function fig = plotITI(obj, methodUsed, plotName, Elec, BaselineCorrect, plotComb, grouping, thresArea)
            %% [Quality, figInfo, tbl] = plotITI(obj, plotName, Elec, plotThis, plotComb, grouping, trangeTopo, TargetOrResponse, MorletOrsFFT, AverageOrSlope)
            % function will extract topoplot to get best electrodes, if
            % Elec is not defined. Topoplot uses trangeTopo to identify the
            % range and you can either choose TargetOrResponse and
            % AverageOrSlope. It further plots target and response-locked
            ...
            for indPP = 1:length(obj.ppNames)
                ...
                for indCond = 1:within.numCond
                    ...
                    %                     tmp = squeeze(removeWhole(:,obj.eeg.epochPlot> max(obj.experiment{1}{1}.ITI).*-1000 &...
                    %                         obj.eeg.epochPlot < 0, currCond) - indivMeanWhole(indPP,obj.eeg.epochPlot> max(obj.experiment{1}{1}.ITI).*-1000 &...
                    %                         obj.eeg.epochPlot < 0)   + grandMeanWhole(obj.eeg.epochPlot> max(obj.experiment{1}{1}.ITI).*-1000 &...
                    %                         obj.eeg.epochPlot < 0));
                    %
                    %                     stdWhole(indCond, indPP) = nanstd(tmp(:));
                    ...
                    if BaselineCorrect == 3
                        ...
                        Threshold(indCond, indPP) =   nanmean(nanmean(removeResponse(:, ...
                            ...
                            currCond),3),2); %  & obj.eeg.responseEpoch <= thresArea(2)
                    else
            ...
            %            figure, plot(obj.eeg.responseEpoch, squeeze(nanmean(currResponse,4))-nanmean(Threshold,2)')
            %% %%%%%%%%%%%%%%%%%%%%%    timeSeries    %%%%%%%%%%%%%%%%%%%%%%%%%%
            % plot timeSeries seperately for fast/slow and no response for
            % both condition both Epoch locked at stimulus onset and
            % response.
            % figInfo{1} = figure('units','normalized','outerposition',[0 0 1 1]); hold on;
            ...
            for indGroup = 1:length(groups)
                ...
                for indBetween = unique(between.Table)
                    ...
                    for indCond = 1:numCond
                        ...
                        cutOut = [obj.eeg.epochPlot > max(obj.stim.lengthITI)*-1000 & obj.eeg.epochPlot < obj.stim.RTdeadLine(end)*1000];
                        
                        %currcond = all( posCombination == currposCombination(indCond,:),2);
                        ...
                        %                         cutOut  = isnan(meanWhole);
                        %                         plotTimeSeries(cutOut) = [];
                        %                         meanWhole(cutOut) = [];% meanWhole = fliplr(meanWhole);
                        %                         CIWhole(cutOut) = []; %CIWhole = fliplr(CIWhole);
                        ...
                        % rtime = obj.eeg.targetEpoch(obj.eeg.targetEpoch > obj.stim.RTCutOff);
                        % plot(rtime(logical(hypoTarget)),ones(sum(hypoTarget),1)*-5+indCond,'Color', currColor(acc,:))
                        %{
                        grid on
                        xstuff = [fliplr(0:-1000:-8000)];
                        xlim([xstuff(1) 500])
                        xticks(xstuff)
                        xticklabels(xstuff./1000)
                        xlabel ('Time before target onset (s)')
                        ax1 = gca;
                        ...
                        startITI = -1 * (max(obj.stim.lengthITI)*1000  - (obj.stim.RTdeadLine(end)*1000 - obj.stim.duration));
        ...
        function [perGoodTrials, ERPTarget, ERPResponse, ERPFalseAlarm] = sortERPs(obj, ERP, indPP, indWhich, artifactReject)
            ...
            if perGoodTrials > 0.3 % conditional only when 30% of the trials are left.
                    ...
                    for indEpoch = 1:length(hitsTrial)
                        if any(hitsTrial(indEpoch,:))
                            for indHit = find(hitsTrial(indEpoch,:) == 1)
                                currtrangeResponse = obj.behaviour{indPP}.indReaction(indEpoch,indHit) + obj.eeg.responseEpoch./(1000/obj.eeg.SampleRate);
                ...
                if (indWhich == 0  || indWhich == 3) && ~isempty(obj.eeg.timing)
                    ...
                    for indEpoch = 1:length(FalseAlarms)
                        ...
                        if ~isempty(obj.eeg.timing)
                            for indFA = 1:length(currFA)
                                
                                if currFA(indFA)*1000 + obj.eeg.responseEpoch(1) < obj.eeg.epochPlot(1)
                                    continue
                                else
                                    
                                    currFAtrange = obj.eeg.epochPlot >= currFA(indFA)*1000  + obj.eeg.responseEpoch(1) &...
                                        obj.eeg.epochPlot <= currFA(indFA)*1000 + obj.eeg.responseEpoch(end)+1000;
                                    ...
                                    %{
                                    forBaseline = ERPFalseAlarm(:,~isnan(ERPFalseAlarm(:,:,indEpoch, indFA)),indEpoch, indFA);
                                    
                                    if size(forBaseline,2) > 100
                                        
                                        forBaseline = nanmean(forBaseline(:,1:100),2);

                                        checkArtifact = nanmean( ERPFalseAlarm(:,:,indEpoch, indFA) - forBaseline);
                                        
                                        if  abs(checkArtifact) >= obj.eeg.artifactThres
                                            ERPFalseAlarm(:,:,indEpoch, indFA) = NaN;
                                        end
                                    else
                                        ERPFalseAlarm(:,:,indEpoch, indFA) = NaN;
                                    end
        ...
        function figInfo = plotAverageBehaviour(obj, plotName, rm, plotComb)
            ...
            for indGroup = 1:length(groups)  % first set the xy/group/seperatedPlots
                ...
                if figInfo.YLim(2) > 150
                    ...
                    yticks(0:200:figInfo.YLim(2))
                    yticklabels((0:200:figInfo.YLim(2))./1000)
        ...
        function [figHandle, onlyLegend] = plotRThistogram(obj, plotComb, grouping)
            ...
            for indPP = 1:length(obj.ppNames)
                ...
                RT          = reshape(obj.behaviour{indPP}.RT.*1000,[],1);
                ...
                for indCond = 1:length(posCombination)
                    ...
                    if obj.DetectOrDisc
                        correct(1:length(currRT),indPP,indCond) = obj.behaviour{indPP}.Correct(currCondHit,:) & currRT > obj.stim.RTCutOff.*1000 & currRT < obj.stim.RTdeadLine(end)*1000;
                        errors(1:length(currRT),indPP,indCond)  = ~(obj.behaviour{indPP}.Correct(currCondHit,:)) & currRT > obj.stim.RTCutOff.*1000 & currRT < obj.stim.RTdeadLine(end)*1000;
                    else
            ...
            % put seperately.
            timeSeries = 0:50:obj.stim.RTdeadLine(end)*1000;
            ...
            for indGroup = 1:length(figHandle)
                ...
                xstuff = [fliplr(0:-500:obj.eeg.targetEpoch(1)) 500:500:obj.eeg.targetEpoch(end)];
                ...
                %                 for indCond = 1:size(currInformation,1)
                %                     acc = acc +1;
                %                     line([meanRT(acc) meanRT(acc)], [0 figAxis], 'Color', obj.figLayOut.colours(acc,:), 'LineWidth', 1);
                %                 end
        ...
        function figHandle = plotIndividualTimeSeries(obj, indPP, Target, Response, plotName, plotComb, grouping)
            ...
            if sum(plotComb == 0)
                otherComb = plotComb(plotComb ~= 0);
                VartrialMatrix  = [obj.behaviour{indPP}.binRT obj.behaviour{indPP}.trialMatrix(:,otherComb)];
                
                unqCond =  unique(VartrialMatrix, 'rows');
                unqCond = unqCond(~any(isnan(unqCond),2),:);
                for indCond = 1:size(unqCond,1)
                    meanRTs(indCond, indPP) = nanmean(obj.behaviour{indPP}.RT(all(VartrialMatrix == unqCond(indCond,:),2)));
                end
                
            elseif ~isempty(plotComb)
                VartrialMatrix  = obj.behaviour{indPP}.trialMatrix(:,plotComb);
            else
            ...
            if grouping ~= 0
                groups = unique(VartrialMatrix(:,plotComb == grouping));
                groups(isnan(groups)) = [];
            else
                groups = 1;
            end
            ...
            for indGroup = 1:length(groups)
                ...
                if grouping ~= 0
                    posCombination = unique(VartrialMatrix(~isnan(obj.behaviour{indPP}.RT(:,1)) & VartrialMatrix(:,plotComb == grouping ) == groups(indGroup),:), 'rows');
                else
                    posCombination = unique(VartrialMatrix(~isnan(obj.behaviour{indPP}.RT(:,1)),:), 'rows');
                end
                posCombination(any(isnan(posCombination),2), :) = [];
                ...
                try
                    for indCond = 1:size(posCombination,1)
                        % plot evidence onset
                        subplot(1,3,1); hold on;
                        currCond = sum(VartrialMatrix == posCombination(indCond,:),2) == size(posCombination,2);
            ...
            colorInd = 1;
            if sum(plotComb == 0)
                posCombination = unique(posCombination(:,1));
            end
            
            for indCond = 1:length(posCombination)
                if sum(plotComb == 0)
                    currCond = VartrialMatrix(:,1) == posCombination(indCond);
                else
                    currCond = sum(VartrialMatrix == posCombination(indCond,:),2) == 2;
                    colorInd = indCond;
                end
                
                subplot(1,3,1); hold on;
                meanPlot = nanmean(Misses(:,:,currCond),3);
                if all(~isnan(meanPlot))
                    SMEPlot  = nanstd(Misses(:,:,currCond), [],3)./sqrt(sum(currCond));
                    currPlot = shadedErrorBar(obj.eeg.targetEpoch, meanPlot, SMEPlot);
                    currPlot.mainLine.Color  = currColor(colorInd,:); currPlot.mainLine.LineWidth = 1; currPlot.mainLine.LineStyle = '-';
                    currPlot.patch.FaceAlpha = 0; currPlot.edge(1).LineStyle = 'none'; currPlot.edge(2).LineStyle = 'none';
                end
                
                subplot(1,3,2); hold on;
                meanPlot = nanmean(falseAlarms(:,:,currCond),3);
                if all(~isnan(meanPlot))
                    SMEPlot  = nanstd(falseAlarms(:,:,currCond), [],3)./sqrt(sum(currCond));
                    currPlot = shadedErrorBar(obj.eeg.responseEpoch, meanPlot, SMEPlot);
                    currPlot.mainLine.Color  = currColor(colorInd,:); currPlot.mainLine.LineWidth = 1; currPlot.mainLine.LineStyle = '-';
                    currPlot.patch.FaceAlpha = 0; currPlot.edge(1).LineStyle = 'none'; currPlot.edge(2).LineStyle = 'none';
                    currPlot.mainLine;
                ...
                if sum(plotComb == 0)
                    colorInd = colorInd + length(posCombination);
                end
            end
                    %}
                    ...
                    if sum(plotComb == 0)
                        lineSpecs = {'-', '-.'};
                        for indCond = 1:size(unqCond,1)
                            line([nanmean(meanRTs(indCond,:)) nanmean(meanRTs(indCond,:))]*1000, currAx.YLim, 'Color', obj.figLayOut.colours(indCond,:),...
                                'LineStyle', lineSpecs{unqCond(indCond,1)} ,'LineWidth', obj.figLayOut.lineWidth)
                        end
                        %line([nanmean(slowRT) nanmean(slowRT)]*1000, currAx.YLim, 'Color', 'k', 'LineStyle', '-.' ,'LineWidth', obj.figLayOut.lineWidth)
                    end
                    
                    if obj.DetectOrDisc
                        line([nanmedian(obj.behaviour{indPP}.RT(:))  nanmedian(obj.behaviour{indPP}.RT(:))]*1000, [yMinPlots yMaxPlots], 'Color', 'k', 'LineWidth', 1);
                    elseif any(plotComb == 0)
                        line([nanmedian(obj.behaviour{indPP}.RT(obj.behaviour{indPP}.binRT == 1)) nanmedian(obj.behaviour{indPP}.RT(obj.behaviour{indPP}.binRT == 1))]*1000, [yMinPlots yMaxPlots], 'Color', 'k', 'LineWidth', 1);
                        line([nanmedian(obj.behaviour{indPP}.RT(obj.behaviour{indPP}.binRT == 2))  nanmedian(obj.behaviour{indPP}.RT(obj.behaviour{indPP}.binRT == 2))]*1000, [yMinPlots yMaxPlots], 'Color', 'k', 'LineWidth', 1, 'LineStyle', '-.');
                    end
                    ...
                    %                 legend(legendThis, num2str(posCombination))
                    ...
                    p_diff = p(3) * (length(-600:200)/diff([obj.eeg.targetEpoch(1) obj.eeg.targetEpoch(end)]));
                    p(3) = p(3) - p_diff;
                    p(1) = p(1) + p_diff/2;
                    set(gca, 'Position', p);
        ...
        function fig = plotAverageTimeseries(obj, Target, Response, falseAlarms, baselineCorrect, plotName, plotComb, grouping, plotThis)
            ...
            faEpoch = obj.eeg.epochPlot (obj.eeg.epochPlot >= obj.eeg.responseEpoch(1) & obj.eeg.epochPlot <= obj.eeg.responseEpoch(end)+1000);
            ...
            for indPP = 1:length(obj.ppNames)
                ...
                for indCond  = 1:size(posCombination,1)
                    ...
                    currRT = reshape(obj.behaviour{indPP}.RT(currCond,:).*1000,[],1);
...
%
%                         %
%                         % check dimensions and if RT are actually good with CPP
%                         RT = obj.behaviour{indPP}.RT;
%                         RT = reshape(RT.*1000,[],1);
%                         for indCond = 1:size(posCombination,1)
%                             % remove all conditions that didn't have a
%                             % response, e.g. the nans.
% %                             currCond = find(sum(VartrialMatrix(1:length(obj.behaviour{indPP}.RT),:) == posCombination(indCond,:),2) == size(posCombination, 2) & ...
% %                                 any(obj.behaviour{indPP}.RT > obj.stim.RTCutOff,2));
%                              currCondHits = sum(allTrialMatrix(1:length(obj.behaviour{indPP}.Early),:,indPP) == posCombination(indCond,:),2) == size(posCombination, 2) & ...
%                              any(obj.behaviour{indPP}.RT > obj.stim.RTCutOff,2) &...
%                                 ~obj.behaviour{indPP}.Misses & ...
%                                 ~obj.behaviour{indPP}.Early;
%
%                             currTarget(:,:, indCond, indPP) = nanmean(Target(:,:, currCondHits), 3);
%
%                             currResponse(:,:, indCond, indPP) = nanmean(Response(:,:,currCondHits, indPP),3);
%
%                             %                         currCond = find(sum(VartrialMatrix == posCombination(indCond,:),2) == size(posCombination, 2));
%                             %                         keyboard; currFA(:,:, indCond, indPP) = nanmean(removeFA(:,:,currCond,:),3);
%
%                         end
%                         %}
%                     else
            ...
            if ~isempty(obj.figLayOut.targetLim)
                ...
                xstuff.Target = [fliplr(0:-500:obj.eeg.targetEpoch(1)) 500:500:obj.eeg.targetEpoch(end)];
            end
            
            if ~isempty(obj.figLayOut.responseLim)
                xstuff.Response = obj.figLayOut.responseLim;
            else
                xstuff.Response = [-500 0 200];
            end
            ...
            for indGroup = 1:length(groups)
                ...
                for indBetween = unique(between.Table)
                    ...
                    for indCond = 1:numCond
                        ...
                        % but right now not
                        %{
                        [~, p] = permtest(squeeze(currTarget(:,obj.eeg.targetEpoch > obj.stim.RTCutOff, indCond, betweenTable == indBetween))',0,10000);
                        hypoTarget = fdr_new(p,0.05); %FDR correct
                        %}
                        ...
                        % rtime = obj.eeg.targetEpoch(obj.eeg.targetEpoch > obj.stim.RTCutOff);
                        % plot(rtime(logical(hypoTarget)),ones(sum(hypoTarget),1)*-5+indCond,'Color', obj.figLayOut.colours(acc,:))
                        
                        grid on
                        
                        xlim([xstuff.Target(1) xstuff.Target(end)])
                        xticks(xstuff.Target)
                        xticklabels(xstuff.Target./1000)
                        ...
                        xticklabels( xstuff.Response./1000)
                        ...
                        if obj.stim.FA && plotThis == 2
                            ...
                            xlim([-800 1000])
                            xticks([-800 0:400:1000])
                            xticklabels([-.8 0:0.4:1])
                            grid on
                        ...
                        if plotThis == 2 && all(~isnan(plotMisses))
                            ...
                            grid on
                            tmpXstuff = [fliplr(0:-500:obj.eeg.targetEpoch(1)) 500:500:obj.eeg.targetEpoch(end)];
                            xlim([obj.eeg.targetEpoch(1) obj.eeg.targetEpoch(end)])
                            xticks(tmpXstuff)
                            xticklabels(tmpXstuff./1000)
                            xlabel ('Time from target (s)')
                            ax1 = gca;
        ...
        function figInfo = plotAverageSurface(obj, Target, plotName, grouping, plotComb)
            ...
            for indCond = groups
                ...
                hold on, plot(currRT*1000, 1:length(currRT), 'k', 'LineWidth', obj.figLayOut.lineWidth);
                xlabel ('Time from target (s)')
            ...
            for indFig = 1:length(figInfo)
                ...
                set(figInfo{indFig}, 'ydir','normal','xlim',[xstuff(1) obj.eeg.targetEpoch(end)], 'XTick', xstuff,...
                    'XTickLabel', xstuff./1000, 'YTick', xstuff, 'clim', limits)
        ...
        function figInfo = plotAverageTopo(obj, topoData, plotName, channels, grouping, AverageOrSlope)
            %% topoplot
            if nargin < 4
                channels = [];
            end
            ...
            for indPlot = 1:length(plotFig)
                ...
                elseif AverageOrSlope == 2
                    ...
                    limits = [floor(min(getLim(1,:))/0.05)*0.05 ceil(max(getLim(2,:))/0.05)*0.05];
                    if limits(1) == 0; limits(1) = -0.1; end
                    if limits(2) == 0; limits(2) = 0.1; end
        ...
        function [figHandle, figInfo] = plotDifferenceTopo(obj, plotName, Elec, plotComb, trangeTopo, TargetOrResponse, AverageOrSlope,...
            ...
            if ~exist(currTopo, 'file')
                ...
                for indPP = 1:length(obj.ppNames)
                    ...
                    elseif AverageOrSlope == 2
                        ERPTopo(1,:,:,indPP) = squeeze(nanmean(diff(tmpERP(:,trangeTopo,:,1),[],2)./diff(obj.eeg.targetEpoch(trangeTopo),[],2),2));
                    end
                end
            ...
            for indGroup = unique(betweenTable)
                h{indGroup} = figure('units','normalized','outerposition',[0 0 1 1]); hold on;
                
                for indTopo = 1:size(newTopo,1)
                    acc = acc+1;
                    subplot(ceil(numCond/2), 2, indTopo)
                    
                    if isempty(Elec)
                        topoFigInd{acc} = topoplot(nanmean(squeeze(newTopo(indTopo,:,betweenTable == indGroup)),2),obj.eeg.chanlocs, 'electrodes', 'labels');
                    else
                        topoFigInd{acc} = topoplot(nanmean(squeeze(newTopo(indTopo,:,betweenTable == indGroup)),2),obj.eeg.chanlocs, 'electrodes', 'labels');
            ...
            for indFig = 1:length(h)
                ...
                for indTopo = 1:size(newTopo,1)
                    ...
                    cb = colorbar('eastoutside');
                    cb.Ticks = [limits(1) 0 limits(2)];
                    title(num2str(posCombination(indTopo,:)));
%
%                     if ~exist('figInfo', 'var')
%                         plotSave(gcf, [plotName '_Topo_' num2str(indPlot) '.png'], currFolder, [obj.figLayOut.saveDim(1) obj.figLayOut.saveDim(1)]);
%                     end
            ...
            for indGroup = unique(between.Table)
                ...
                for indSub = 1:size(AllSub,1)
                    ...
                    try
                        ...
                        elseif AverageOrSlope == 2
                            limits = [round(min(getLim(1,:))/0.005)*0.005 round(max(getLim(2,:))/0.005)*0.005];
                            if limits(1) == 0; limits(1) = -0.01; end
                            if limits(2) == 0; limits(2) = 0.01; end
                        end
            ...
            figInfo = topoFigInd;
            figHandle = h;
            % plot diff/diff plots :)
            keyboard
            for indGroup = unique(betweenTable)
                h{indGroup} = figure('units','normalized','outerposition',[0 0 1 1]); hold on;
                
                diffdiffTopo = diffTopo(1,:,:) -  diffTopo(2,:,:);
                
                acc = acc+1;
                
                if isempty(Elec)
                    topoFigInd{acc} = topoplot(nanmean(squeeze(diffdiffTopo(:,:,betweenTable == indGroup)),2),obj.eeg.chanlocs, 'electrodes', 'labels');
                else
                    topoFigInd{acc} = topoplot(nanmean(squeeze(diffdiffTopo(:,:,betweenTable == indGroup)),2),obj.eeg.chanlocs, 'electrodes', 'labels');
                ...
                figure(h{indFig});
                    if AverageOrSlope == 1
                        if obj.eeg.applyCSD
                            ...
                            limits = [round(min(getLim(1,:))/1)*1 round(max(getLim(2,:))/1)*1];
                            if limits(1) == 0; limits(1) = -1; end
                            if limits(2) == 0; limits(2) = 1; end
                        end
                    elseif AverageOrSlope == 2
                        limits = [round(min(getLim(1,:))/0.05)*0.05 round(max(getLim(2,:))/0.05)*0.05];
                        if limits(1) == 0; limits(1) = -0.1; end
                        if limits(2) == 0; limits(2) = 0.1; end
                    end
                    ...
                    cb.Ticks = [limits(1) 0 limits(2)];
                    if length(plotComb) == 1
                        title(obj.figLayOut.legTitle{plotComb});
                    else
                        title(obj.figLayOut.legNames{plotComb(1)}(indSub));
                    end
        ...
        function [shortFFT, timeSeries] = shortfft(obj, ERP, intFreq)
            ...
            stepSize    = ceil(0.050/(1/Fs)); % 50 ms steps
            windowWidth = floor(L/2);
            timeSeries  = windowWidth+1:stepSize:length(obj.eeg.epochPlot)-windowWidth;
            
            %{
            freqRange  = [0:winLength-1]*parameters.eeg.SampleRate/winLength; % the frequency
            
            if isfield(obj.stim, 'freqSSVEP') && length(intFreq) == 1
                intFreq = find(freqRange == intFreq);
            elseif isfield(obj.stim, 'freqSSVEP') && length(intFreq) > 1
                intFreq = find(freqRange >= intFreq(1) & freqRange <= intFreq(end) & freqRange ~=(obj.stim.freqSSVEP));
            elseif length(intFreq)== 1
                [~, intFreq] = max(abs(freqRange - intFreq(1)));
            else
                intFreq = find(freqRange >= intFreq(1) & freqRange <= intFreq(end));
            end
            %}
            ind = 1;
            for tt = timeSeries
            ...
            Fs = obj.eeg.SampleRate; % Sampling frequency
            L  = sum(obj.eeg.epochPlot >= 0 & obj.eeg.epochPlot <= windowSize);    % samples
            dF = Fs/L;               % hertz per sample
            f  = 0:dF:Fs-dF + (dF/2)*mod(L,2);      % hertz
            intF = f >= intFreq(1) & f <= intFreq(end);
            
            if isfield(obj.stim, 'freqSSVEP')
                intF(f >= obj.stim.freqSSVEP -1 & f <= obj.stim.freqSSVEP +1) = 0;
            end
            stepSize = 5;% 30*(1/Fs)*1000;
            windowWidth = round((windowSize/1000)/(1/Fs)/2);
            timeSeries  = windowWidth+1:stepSize:length(obj.eeg.epochPlot)-windowWidth;
            ...
            ind = 1;
            for tt = timeSeries
                %                 slidingWindow = find(obj.eeg.epochPlot(tt) == (tt - winLength/2)):find(obj.eeg.epoch{indLock} ==(tt + winLength/2))-1;
                fourTrans	  = abs(fft(ERP(:, tt-windowWidth:tt+windowWidth, :),[],2));%./(winLength/2);
                shortFFT(:,:, ind,:) = fourTrans(:, intF, :);
                ind = ind+1;
            end
            %}
        end
...
if ~exist('numCycles', 'var')
                normalFreq   = 7:0.5:30;	% mu and ERD range
                range_cycles = [8 15];
            ...
            figure
            indPP = 1;
            subplot(2,2,1); plotOutput = squeeze(nanmean(nanmean(output(:,:,:,obj.behaviour{1}.Reaction == 2 & obj.behaviour{indPP}.trialMatrix(:,3) == 0),4),1));
            contourf(obj.eeg.epochPlot(obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),frex,...
                plotOutput(:,obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),40,'linecolor','none')
            colormap('jet')
             subplot(2,2,2); plotOutput = squeeze(nanmean(nanmean(output(:,:,:,obj.behaviour{1}.Reaction == 2 & obj.behaviour{indPP}.trialMatrix(:,3) == 1),4),1));
            contourf(obj.eeg.epochPlot(obj.eeg.epochPlot >= -1500 & obj.eeg.epochPlot < 2000),frex,...
                plotOutput(:,obj.eeg.epochPlot >= -1500 & obj.eeg.epochPlot < 2000),40,'linecolor','none')
            colormap('jet')
            
            subplot(2,2,2); plotOutput = squeeze(nanmean(nanmean(output(:,:,:,obj.behaviour{indPP}.trialMatrix(:,1) == 2 & obj.behaviour{indPP}.trialMatrix(:,2) == 70),4),1));
            contourf(obj.eeg.epochPlot(obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),frex,...
                plotOutput(:,obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),40,'linecolor','none')
            colormap('jet')
               subplot(2,2,3); plotOutput = squeeze(nanmean(nanmean(output(:,:,:,obj.behaviour{indPP}.trialMatrix(:,1) == 1 & obj.behaviour{indPP}.trialMatrix(:,2) == 70),4),1));
            contourf(obj.eeg.epochPlot(obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),frex,...
                plotOutput(:,obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),40,'linecolor','none')
            colormap('jet')
            subplot(2,2,4); plotOutput = squeeze(nanmean(nanmean(output(:,:,:,obj.behaviour{indPP}.trialMatrix(:,1) == 2 & obj.behaviour{indPP}.trialMatrix(:,2) == 25),4),1));
            contourf(obj.eeg.epochPlot(obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),frex,...
                plotOutput(:,obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),40,'linecolor','none')
            colormap('jet')
            ...
            indPP = 1;
            subplot(2,2,1);
            contourf(obj.eeg.responseEpoch,freqRange{3},...
                response(),40,'linecolor','none')
            colormap('jet')
            subplot(2,2,2);
            contourf(obj.eeg.epochPlot(obj.eeg.epochPlot >= -1000 & obj.eeg.epochPlot < 2000),frex,...
                response,40,'linecolor','none')
            ...
            set(gca,'ydir','normal', 'YTick', frex, 'YTickLabel', frex)%'clim',[-3 3]  'xlim',[obj.eeg.epochPlot(300) obj.eeg.epochPlot(end-300)],...
            'xTickLabel', round(obj.eeg.epochPlot(300)):500:round(obj.eeg.epochPlot(end-300))
            title('Wavelet')
            axis square
